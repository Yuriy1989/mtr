...
export const handleExportToExcel = async (tableData) => {
  ...
  worksheet.getColumn(1).width = 7.5; // № п/п
  worksheet.getColumn(2).width = 41.4; // Наименование МТР
  worksheet.getColumn(3).width = 20; // Код МТР
  worksheet.getColumn(4).width = 7; // Ед. изм.
  worksheet.getColumn(5).width = 14; // Кол-во
  worksheet.getColumn(6).width = 27; // Структурное подразделение Отправитель
  worksheet.getColumn(7).width = 26; // Структурное подразделение Получатель (промежуточный склад)
  worksheet.getColumn(8).width = 22; // № Требования-Накладной на перемещение
  worksheet.getColumn(9).width = 22; // Определение срочности перевозки
  worksheet.getColumn(10).width = 30; // Структурное подразделение Получатель
  worksheet.getColumn(11).width = 23; // Основание для разнарядки
  worksheet.getColumn(12).width = 39; // Наименование объекта ремонта
  worksheet.getColumn(13).width = 31; // Особые отметки
  worksheet.getColumn(14).width = 25; // Примечание

  // Добавляем пустые строки для смещения заголовков в 10 и 11 строки
  for (let i = 0; i < 9; i++) {
    worksheet.addRow([]);
  }
  ...
  // Добавляем номера колонок после шапки
  const columnNumbers = [
    ...Array.from(
  ...
  // Преобразуем данные с учетом колонок
  tableData.forEach((row, index) => {
    const rowData = [
      index + 1, // Нумерация строк
